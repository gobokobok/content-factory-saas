...
**Date:** 2026-09-19
**Status:** ACTIVE
**Decision:** `_motion_vf_prefix` (`src/ffmpeg_builder.py`) now scales pan stills with `scale={out_w}:{out_h}:force_original_aspect_ratio=increase` instead of `scale=-2:{out_h}`, so the scaled width is always >= the crop window. Landscape stills are unchanged (height is still the binding dimension, so the same overflow remains to pan across); a narrow portrait now has zero lateral headroom and the pan collapses to a static crop rather than failing.
**Root cause.** `scale=-2:1920` fixes the height and lets the width follow. An operator-uploaded 1536x2752 portrait (aspect 0.558, narrower than 9:16's 0.5625) scales to 1071.6 -> 1072px, then `crop=1080:1920` cannot fit: `Invalid too big or non positive size for width '1080' or height '1920'` -> `Error reinitializing filters` -> exit 1 for the whole render. Deterministic, single process, no concurrency involved. Introduced with the D081 pan effects, which assumed a still is always at least as wide as the frame after scale-to-height.
**How D090 got it wrong (recorded so it is not repeated).** Three PROD renders and one local repro all failed at the same `Parsed_crop_1` line. The log interleaves stderr from up to four concurrent ffmpeg processes, so I attributed the failure to scene 8 by log position and never ran scene 6's own filter standalone. Every "reproduction" of a threading race was actually this deterministic bug: my Docker repro failed on **scene 6, not 8**, and I read that as "whichever process loses the race" instead of as evidence against the theory. Two deploys (v0.23.3, v0.23.4) shipped `-threads` / `-filter_threads` caps on that basis; neither changed the outcome. The operator found the cause first by deleting the manually uploaded scene-6 image and re-rendering successfully. Lesson: reproduce each scene's exact command in isolation before theorising about concurrency.
**Not reverted here:** the `FFMPEG_SCENE_THREADS` caps from D090 remain in the code. They are harmless but unjustified, and may slow renders slightly; removing them is a separate decision.
**Test coverage:** `test_pan_never_scales_narrower_than_the_crop_window` runs real ffmpeg on a 1536x2752 still with both pan directions (fails on the pre-fix code, passes after); two existing tests that pinned the buggy `scale=-2:1920` string were updated.
**No new dependency.**
**Implemented by:** operator observation ("i removed the manually uploaded image (scene 6) and it rendered"), 2026-09-19.
**See:** D081, D090.
...
**Date:** 2026-09-18
**Status:** SUPERSEDED by D091 — the diagnosis below was WRONG. The crash was never thread contention; see D091. The `-threads` / `-filter_threads` caps it introduced are unjustified by the actual cause.
...
**Follow-up (same day):** the v0.23.2 deploy immediately 500'd every render (`AttributeError: 'PlatformSettings' object has no attribute 'FFMPEG_SCENE_THREADS'`). `cf_platform/core/config.py` maintains its own standalone `PlatformSettings` class, independent of `src/config.py`'s `Settings` (D047 — `cf_platform` may not import `src/` outside the legacy adapter), duplicating fields like `FFMPEG_TIMEOUT_SECONDS` / `COLOR_GRADE_PRESET` / `BLUR_FILL_ENABLED` under the same ENV var names. `FFMPEG_SCENE_THREADS` was added only to the former; `cf_platform/interfaces/routes/workers.py`'s render endpoint reads the latter and crashed on every call. Fixed by adding the same field to `PlatformSettings` (v0.23.3). **Why CI stayed green:** the one existing test through this endpoint (`test_render_endpoint_forwards_captions_false`) passes `settings=MagicMock(COLOR_GRADE_PRESET=..., BLUR_FILL_ENABLED=..., FFMPEG_TIMEOUT_SECONDS=...)` — an unlisted attribute on a `MagicMock` silently returns another `MagicMock` instead of raising, so a field missing from only one of the two real settings classes is invisible to a mock-settings test. Added `test_render_endpoint_works_with_real_platform_settings`, which builds a real `PlatformSettings()` and reproduces the PROD crash before the fix (verified: fails on the pre-fix class, passes after).
**Second follow-up (same day):** v0.23.3 deployed cleanly but the operator retried the render on the *same* run and got the *identical* crop-filter crash on scene 8 — `threads=2` visibly correct in the log this time, proving the original diagnosis was incomplete. `-threads` only caps the **encoder's** thread pool. FFmpeg's `-vf` filter graph has its own, entirely separate thread pool controlled by the global option `-filter_threads` (`ffmpeg -h full`: "number of non-complex filter threads") — left unset, it independently defaults to the host's full CPU count. The crash's own log tags say exactly this: `[vf#0:0 @ ...] Task finished with error code: -22` is the **filter** thread pool's worker task failing, not the encoder's — `-threads` was never going to touch it. Both `_render_video_scene` and `_render_image_scene` now also pass `-filter_threads {threads}` as a global option (before `-i`, since it isn't scoped to an output). Same `FFMPEG_SCENE_THREADS` setting drives both flags. Could not reproduce the race locally (ARM build behaves differently under load, and this needs genuine multi-process host contention to surface), so this is a plausible-not-confirmed fix pending the next PROD retry — flagged here rather than claimed as verified.
**See:** D089, D047.
...
**Date:** 2026-09-01
**Status:** ACTIVE
**Decision:** `_render_scene` (`src/ffmpeg_builder.py`) routes on **file extension alone** and no longer consults `clip_type`: anything not in `_IMAGE_EXTS` — an unrecognised extension included — goes to `_render_video_scene`. New `rederive_scene_visual_contract(duration_s, is_video, current_motion_effect)` in `storyboard_worker.py`, called by `studio_upload_scene_asset` via `_rederive_scene_media_contract`, rewrites the storyboard scene's `asset_tier` / `clip_type` / `motion_effect` to match the uploaded media kind. Video → `("video", "hard_cut", None)`; image → the duration-derived still tier floored to `still_motion`, preserving an effect the operator already chose.
**Root cause.** `asset_tier` is derived once, from scene **duration** (`_assign_asset_tier`: ≥6s → video), and `clip_type` / `motion_effect` follow from it. That is the right guess while acquisition is still picking the asset, but the operator can overrule it afterwards and nothing re-derived it: `studio_upload_scene_asset` computed `is_video` purely to choose the `video/` vs `images/` R2 folder, patched the **manifest** entry, and left the **storyboard** untouched. A 3–6s scene holding an operator-uploaded MP4 therefore still claimed to be a still, so `_render_scene` sent it to `_render_image_scene`, whose `-loop 1` is an **image2-demuxer** option. FFmpeg 7 rejects it on mov/mp4 outright — `Option loop not found` / `Error opening input file` / `Error opening input files: Option not found` — and the script exits 1, taking the whole render with it, not just that scene.
**The routing was asymmetric, and that is the general defect.** The extension check already existed but guarded one direction only: a JPEG on a `hard_cut` scene was rescued from the video path (it would have produced a ~1-frame clip), while a video on a still scene was not rescued from the image path. Inverting the condition closes the other half. Any future source of tier/asset disagreement — a re-acquisition, a hand-edited artifact, a storyboard regenerated after upload — is now inert at render time rather than fatal.
**Why both halves ship together.** The routing fix alone makes existing broken runs renderable without re-uploading, but leaves Studio showing a `video` scene with an `image` badge and a live motion dropdown that does nothing. The storyboard fix alone makes the UI honest but leaves the renderer one stale artifact away from the same crash. Neither is redundant.
**Motion effects are stills-only, and are now cleared rather than ignored.** `_render_video_scene` has never read `motion_effect`, `_derive_motion_effect` has always returned `None` for the video tier, and the Studio table already renders a disabled `—` for video scenes. The rule was implemented everywhere but could still be *contradicted by stored data*; an effect on a video scene is now nulled at the point the scene becomes a video, so what the storyboard says and what the render does cannot diverge.
**An unchanged contract writes no artifact version.** `_rederive_scene_media_contract` returns `None` when the scene already describes the uploaded media kind. Every reader resolves "latest" (D086) and the version history is the operator's audit trail, so a re-upload of the same media kind must not burn a version. The write is deliberately **not** best-effort — unlike the adjacent `trace_events.record` — because leaving the storyboard describing a still while an MP4 sits in the manifest is precisely the inconsistency this entry exists to remove.
**Evidence.** PROD run `cb9eca1e-45e4-47c2-a7a0-ba9df097ebf4`, two identical failed render attempts. Its `ffmpeg_log.txt` shows scenes 1–4 and 6–7 (all `.jpeg`) encoding cleanly and only `video/scene_05_op.mp4` — the `_op` suffix marks an operator upload — failing. Reproduced locally by feeding that same file to `_render_image_scene`, which emits the three PROD error lines verbatim. Verified fixed by running the new `_render_scene` output through real FFmpeg: exit 0, 1080×1920, exactly 100 frames / 4.000s / 25fps, matching the frame-exactness the concat step depends on.
**Test coverage:** `tests/cf_platform/test_d089_operator_video_on_still_scene.py` (15) pins both guards independently — asserting `-loop` never precedes a video, that all five motion effects produce a byte-identical command on a video scene, that the pre-existing image-on-`hard_cut` guard survives the inversion, and that an mp4 upload rewrites the scene while a matching image upload writes nothing. Confirmed to fail against the pre-fix code (6 routing tests, 2 endpoint tests). `test_p10_s2_asset_override.py`'s upload tests gained a storyboard read stub, since the endpoint now reads both artifacts.
**No new dependency.**
**Implemented by:** operator chat report ("FFmpeg exited 1 for run cb9eca1e…"; "i want the video be rendered as video… if any motion effects are specified, those shall be ignored"), 2026-09-01.
**See:** D081, D086, D087.
