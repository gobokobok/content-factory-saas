...
> - **Sprint P1 complete** (16/16 pts). **Sprint P2 complete** (16/16 pts) — Lineage & Observability Store. P2-S1 (smoke test passed), P2-S2 (smoke test deferred), P2-S3 (smoke test passed on Railway DEV), P2-S4 (smoke test deferred), P2-S5 (smoke test passed via TestClient) — all done. **PROD follow-up still open:** confirm `content-factory-prod` has `DATABASE_URL`/Postgres set before P2 work ships to PROD. **Current sprint:** P3 — Telegram Trigger + Discovery Worker. **P3-S1 done** (smoke test deferred — requires `TELEGRAM_BOT_TOKEN`/`TELEGRAM_WEBHOOK_SECRET` on Railway DEV). **Active story:** P3-S2 (Discovery worker v1 + source adapters).
