-- Content Factory v2 Platform — Postgres metadata-index schema (design draft)
--
-- Status: DESIGN ONLY (P0-S4). Not applied by any code path yet — zero runtime
-- behavior per P0 architectural law 7. P2-S2 turns this into the first
-- numbered migration file (cf_platform/db/migrations/0001_init.sql) and wires
-- it up via the chosen migration runner (raw SQL — see DECISIONS.md D048).
--
...
-- See docs/v2_platform_plan.md §6 for the narrative spec.
...
CREATE INDEX idx_runs_block_status ON runs (block, status);
...
CREATE INDEX idx_artifacts_prompt_version ON artifacts (prompt_version);
...
CREATE INDEX idx_worker_executions_run_id ON worker_executions (run_id);
CREATE INDEX idx_worker_executions_prompt_version ON worker_executions (prompt_version);
CREATE INDEX idx_worker_executions_worker_version ON worker_executions (worker_version);
...
CREATE INDEX idx_trace_events_source ON trace_events (source);
...
CREATE INDEX idx_published_videos_external_id ON published_videos (external_id);
CREATE INDEX idx_published_videos_run_id ON published_videos (run_id);
...
CREATE INDEX idx_video_metrics_external_id_captured_at ON video_metrics (external_id, captured_at);
