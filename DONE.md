...
- **Operational follow-up (not blocking):** `TELEGRAM_BOT_TOKEN`/`TELEGRAM_WEBHOOK_SECRET`/`TELEGRAM_ALLOWED_CHAT_IDS` are not yet set on `content-factory-dev`. Once set, an operator runs `TelegramClient(bot_token).register_webhook(webhook_url, secret_token)` once (e.g. via a Python shell or a small ad-hoc script) to point Telegram at `https://content-factory-dev-production.up.railway.app/platform/telegram/webhook`.
**Smoke test:** DEFERRED — requires `TELEGRAM_BOT_TOKEN`/`TELEGRAM_WEBHOOK_SECRET`/`TELEGRAM_ALLOWED_CHAT_IDS` on Railway DEV and a registered webhook. On next deploy with those set: register the webhook, send `/ideas <niche>` from Telegram, confirm the bot replies with the ack text (and that a bare `/ideas` or random text gets the usage/help reply instead, and that a different chat id gets no reply). Covered for now by 27 unit/route tests with `httpx`/`TelegramClient.send_message` mocked.
