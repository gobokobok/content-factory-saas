...
- 927 total passing (was 917). No new ENV vars, no new dependencies (`psycopg`/`psycopg_pool` already added in P2-S1), no DECISIONS.md entry needed.
- AC #2 (artifact bodies stay in R2 only) is satisfied by construction — `PostgresArtifactRepository.record()`'s INSERT only references `r2_key` + lineage columns, never a body/content column. AC #3 (idempotency) is satisfied by the `ON CONFLICT` clauses on both `runs` (upsert) and `artifacts` (no-op on retry of the same version).
- **Sprint P2 next story:** P2-S4 (LangGraph PostgresSaver checkpointer, in progress in parallel) and then P2-S5 (Observability endpoints, depends on P2-S3 — now unblocked).
...
**Smoke test:** DEFERRED — requires deploy to Railway DEV (`content-factory-dev`, `DATABASE_URL` already set per P2-S1). On next deploy, run `POST /platform/echo` twice with the same input and confirm via `psql`: one row in `runs` (status `complete`, upserted not duplicated), one row in `artifacts` (version 1, `r2_key` set, no body column), one row in `worker_executions` per run.
**Promoted to backlog:** none
