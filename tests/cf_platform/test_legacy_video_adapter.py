...
class TestInProcessAdapterHappyPath:
    ...
    @pytest.mark.asyncio
    async def test_trace_event_sources_correct_with_tts(self):
        """With TTS credentials, 6 trace events emitted (tts + 5 pipeline steps)."""
        settings = _make_settings(elevenlabs_api_key="sk-xxx", elevenlabs_voice_id="v1")
        adapter = _make_adapter(settings)
        trace_repo = InMemoryTraceEventRepository()
        run_id = "tts-run"
        ...
        with (
            patch(f"{_MODULE}.R2Client", return_value=mock_storage),
            patch(f"{_MODULE}.generate_tts", new=AsyncMock(return_value=(b"mp3bytes", 3))),
        ...
        events = await trace_repo.list_for_run(run_id)
        assert [e.source for e in events] == ["tts", "storyboard", "manifest", "acquisition", "ffmpeg_script", "render"]
...
class TestTTSBehaviour:
    @pytest.mark.asyncio
    async def test_tts_skipped_when_no_elevenlabs_key(self):
        """TTS is not attempted when ELEVENLABS_API_KEY is absent."""
        adapter = _make_adapter(_make_settings(elevenlabs_api_key="", elevenlabs_voice_id=""))
        trace_repo = InMemoryTraceEventRepository()
        run_id = "no-tts-run"
        ...
        with (
            ...
            patch(f"{_MODULE}.generate_tts", new=AsyncMock()) as mock_tts,
            patch(f"{_MODULE}.generate_storyboard", new=AsyncMock(return_value=(_make_storyboard_mock(), _make_validation_mock()))),
            patch(f"{_MODULE}.Storyboard") as mock_storyboard_cls,
            patch(f"{_MODULE}.build_manifest", return_value=mock_manifest),
            patch(f"{_MODULE}.run_acquisition", new=AsyncMock(return_value={"acquired": 1, "failed": 0, "sources": {}})),
    ...
    @pytest.mark.asyncio
    async def test_tts_failure_is_non_fatal(self):
        """TTS failure logs a warning but the pipeline continues and succeeds."""
        settings = _make_settings(elevenlabs_api_key="sk-xxx", elevenlabs_voice_id="v1")
        adapter = _make_adapter(settings)
        trace_repo = InMemoryTraceEventRepository()
        run_id = "tts-fail-run"
        ...
        with (
            patch(f"{_MODULE}.R2Client", return_value=MagicMock()),
            patch(f"{_MODULE}.generate_tts", new=AsyncMock(side_effect=RuntimeError("ElevenLabs 429"))),
            patch(f"{_MODULE}.generate_storyboard", new=AsyncMock(return_value=(_make_storyboard_mock(), _make_validation_mock()))),
            patch(f"{_MODULE}.Storyboard") as mock_storyboard_cls,
            patch(f"{_MODULE}.build_manifest", return_value=mock_manifest),
            patch(f"{_MODULE}.run_acquisition", new=AsyncMock(return_value={"acquired": 1, "failed": 0, "sources": {}})),
        ...
        assert result.status == "complete"
        events = await trace_repo.list_for_run(run_id)
        tts_event = next(e for e in events if e.source == "tts")
        assert tts_event.status == "error"
        assert "ElevenLabs 429" in tts_event.meta["error"]
